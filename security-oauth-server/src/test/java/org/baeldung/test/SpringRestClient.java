...
import com.ecommerce.security.oauth.model.Employee;
...
public class SpringRestClient {
    ...
    public static final String REST_SERVICE_URI = "http://localhost:9081/security-oauth-server";
    
    public static final String AUTH_SERVER_URI = "http://localhost:9081/security-oauth-server/oauth/token";
    
    public static final String QPM_PASSWORD_GRANT = "?grant_type=password&username=admin&password=nimda";
...
//        ResponseEntity<List> response = restTemplate.exchange(REST_SERVICE_URI+"/employee/"+QPM_ACCESS_TOKEN+tokenInfo.getAccess_token(),
...
//    }
    ...
    public static void httpPostRequestWithHeadersAndBody(AuthTokenInfo tokenInfo)
    {
...
//        requestHeaders.add("Authorization", authorizationHeader);
        ...
        Employee employee = new Employee(EMAIL, NAME);


        RestTemplate restTemplate = new RestTemplate();

        HttpHeaders requestHeaders = getHeaders();
        requestHeaders.add("Authorization", "Bearer " + tokenInfo.getAccess_token());
        ...
        if(responseEntity.getStatusCode() == HttpStatus.OK){
            Employee employee_1 = responseEntity.getBody();
            System.out.println("Employee response retrieved ");
        }
    }
...
//    	listAllUsers(tokenInfo);
        ...
        httpPostRequestWithHeadersAndBody(tokenInfo);
